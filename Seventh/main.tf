...
locals {
  ...
  fluentbit_opensearch          = yandex_compute_instance.opensearch[0].network_interface.0.ip_address
}
...
resource "local_file" "fluentbit-opensearch" {
  ...
  content = templatefile("fluent-bit.conf.tftpl", {
    fluentbit_opensearch = local.fluentbit_opensearch
  })
}
...
resource "yandex_vpc_security_group" "custom_sg" {
  ...
  dynamic "ingress" {
    for_each = ["80", "443", "22", "3306", "33060", "4567", "4444", "4568", "6032", "6033", "9200", "9300", "5601", "9250", "9600", "24224"]
    content {
      protocol       = "TCP"
...
resource "terraform_data" "run_ansible" {
  depends_on = [yandex_compute_instance.database, yandex_compute_instance.nginx, yandex_compute_instance.backend, yandex_compute_instance.admin, yandex_compute_instance.opensearch]
